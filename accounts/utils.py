...
def get_gravatar(email, size=80, default='identicon'):
    ...
    gravatar_url += urllib.urlencode({'s': bytes(size),
                                      'd': default})
    return gravatar_url
...
def generate_sha1(string, salt=None):
    ...
    if not salt:
        salt = sha_constructor(bytes(random.randint(10, 20))).hexdigest()[:5]
    hash = sha_constructor(str.encode(salt)+string).hexdigest()
